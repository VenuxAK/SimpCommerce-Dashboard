...
const loadReport = async () => {
  ...
  try {
    const [salesRes, bestRes, payRes] = await Promise.all([
      reportApi.sales({ date_from: dateFrom.value, date_to: dateTo.value }),
      reportApi.bestSellers({ date_from: dateFrom.value, date_to: dateTo.value, limit: 20 }),
      reportApi.paymentMethods({ date_from: dateFrom.value, date_to: dateTo.value }),
    ])
